...
- A multi-word remote command for `ShellTo` is passed as trailing bare words after a literal `--` (e.g. `ShellTo <selector> --ssh-user root -- ae3 status`) — `--execute-command`/`--execute-script`/`--execute-stdin`/`--display-targets` are `ExecuteParallel`/`ExecuteSequence` options, not `ShellTo`'s at all. `ShellTo.fn.sh` should reject those four with a clear error naming the trailing-`--` form. It does not: its option loop handles `--ssh-*` only, so any of the four falls through and is silently folded into the remote-command tokens, surfacing as a confusing `ssh: illegal option` three layers downstream. Open, not yet fixed.

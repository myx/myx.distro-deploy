...
- Piping a script to `ShellTo <selector> -- bash` (or `sh`) as the remote command works well and is a common pattern — `ShellTo` defaults to exactly this (`-t '\`command -v bash || command -v sh\`'`) when no trailing command is given at all.

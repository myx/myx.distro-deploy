...
ScreenTo(){
	...
	local defaultCommand="set -x ; -t command -v screen  >/dev/null 2>&1 && env SHELL=\"\$(command -v bash || command -v sh)\" screen -q -O -U -D -R || command -v screen  >/dev/null 2>&1 && exec bash || exec sh'"
